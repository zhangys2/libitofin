...
//! here, starting with [`Euribor`] and [`USDLibor`] / [`Libor`], plus the named
//! [`OvernightIndex`] concretes [`Eonia`], [`Sofr`], and [`Estr`]. Items are
//! re-exported flat, so the index is `indexes::ibor::Euribor` rather than
