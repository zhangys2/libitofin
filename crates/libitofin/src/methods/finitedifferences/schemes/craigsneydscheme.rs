...
//!
//! Port of `ql/methods/finitedifferences/schemes/craigsneydscheme.{hpp,cpp}`:
//! an explicit update, a θ-weighted directional correction (as in Douglas),
//! then a μ-weighted mixed-term predictor and a second directional pass that
//! still reads the step input `a`.
//!
//! QuantLib's default [`FdmSchemeDesc::craig_sneyd`](crate::methods::finitedifferences::solvers::FdmSchemeDesc::craig_sneyd)
//! uses `θ = ½`, `μ = ½`.
...
/// Craig–Sneyd ADI step (`craigsneydscheme.hpp:40`).
pub struct CraigSneydScheme {
    dt: Option<Time>,
    theta: Real,
    mu: Real,
...
impl CraigSneydScheme {
    /// `CraigSneydScheme(theta, mu, map, bcSet)` (`craigsneydscheme.cpp:32-35`).
    pub fn new(
        theta: Real,
        mu: Real,
        map: SharedMut<dyn FdmLinearOpComposite>,
...
impl Scheme for CraigSneydScheme {
    /// `craigsneydscheme.cpp:64-66`.
    fn set_step(&mut self, dt: Time) {
        self.dt = Some(dt);
    }

    /// `craigsneydscheme.cpp:38-61`.
    ///
    ...
    /// `yt = y0 + μ dt A_mixed(y − a)` and corrects again, still reading `a`
    /// on each directional RHS (unlike Hundsdorfer, which reads the
    /// post-stage-1 `y` and uses the full operator).
    ...
    fn step(&mut self, a: &mut Array, t: Time) -> QlResult<()> {
        ...
        let mut yt = {
            ...
            self.bc_set.apply_before_applying(&mut *map);
            let mut yt = &y0 + &((self.mu * dt) * &map.apply_mixed(&(&y - &*a)));
...
mod tests {
    ...
    fn craig_sneyd(
        ...
        let mut scheme = CraigSneydScheme::new(THETA, MU, map, bc_set);
        scheme.set_step(DT);
        scheme
    }
    ...
    fn a_step_replays_the_cpp_sequence_on_the_black_scholes_operator() {
        ...
        let mut scheme = craig_sneyd(map, Vec::new());
        ...
        let mut yt = &y0 + &((MU * DT) * &oracle.apply_mixed(&(&y - &u)));
        for i in 0..oracle.size() {
    ...
    /// then stage-2 `y0 + μ dt A_mixed(y − a)` (zero here) with a second
    /// directional pass that still reads the input `a`.
    #[test]
    fn a_step_matches_the_closed_form_on_a_diagonal_operator() {
        let mut scheme = craig_sneyd(scaled_composite(&COEFFICIENTS), Vec::new());
    ...
    fn a_step_sets_operator_and_runs_both_bc_apply_cycles() {
        ...
        let mut scheme = craig_sneyd(map, bc_set);
    ...
    fn stepping_before_the_timestep_is_set_fails() {
        let mut scheme =
            CraigSneydScheme::new(THETA, MU, scaled_composite(&COEFFICIENTS), Vec::new());
