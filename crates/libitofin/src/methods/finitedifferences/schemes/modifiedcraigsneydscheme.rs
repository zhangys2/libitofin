...
//!
//! Port of `ql/methods/finitedifferences/schemes/modifiedcraigsneydscheme.{hpp,cpp}`:
//! Craig–Sneyd with an extra `(½ − μ) dt A(y − a)` term on the mixed predictor.
//!
//! QuantLib's default [`FdmSchemeDesc::modified_craig_sneyd`](crate::methods::finitedifferences::solvers::FdmSchemeDesc::modified_craig_sneyd)
//! uses `θ = μ = ⅓`.
...
/// Modified Craig–Sneyd ADI step (`modifiedcraigsneydscheme.hpp:40`).
pub struct ModifiedCraigSneydScheme {
    dt: Option<Time>,
    theta: Real,
    mu: Real,
...
impl ModifiedCraigSneydScheme {
    /// `ModifiedCraigSneydScheme(theta, mu, map, bcSet)`
    /// (`modifiedcraigsneydscheme.cpp:30-33`).
    pub fn new(
        theta: Real,
        mu: Real,
        map: SharedMut<dyn FdmLinearOpComposite>,
...
impl Scheme for ModifiedCraigSneydScheme {
    /// `modifiedcraigsneydscheme.cpp:62-64`.
    fn set_step(&mut self, dt: Time) {
        self.dt = Some(dt);
    }

    /// `modifiedcraigsneydscheme.cpp:36-59`.
    ///
    /// Stage 1 is Craig–Sneyd. Stage 2 builds
    /// `yt = y0 + μ dt A_mixed(y − a) + (½ − μ) dt A(y − a)` and corrects
    /// again, still reading the step input `a`.
    ...
    fn step(&mut self, a: &mut Array, t: Time) -> QlResult<()> {
        ...
        let mut yt = {
            ...
            self.bc_set.apply_before_applying(&mut *map);
            let delta = &y - &*a;
            let mut yt = &y0 + &((self.mu * dt) * &map.apply_mixed(&delta));
            yt = &yt + &(((0.5 - self.mu) * dt) * &map.apply(&delta));
            self.bc_set.apply_after_applying(&mut yt);

            for i in 0..map.size() {
                let rhs = &yt - &((self.theta * dt) * &map.apply_direction(i, a));
...
mod tests {
    ...
    const THETA: Real = 1.0 / 3.0;
    const MU: Real = 1.0 / 3.0;
    const DT: Time = 0.1;
    const T: Time = 0.25;
    const COEFFICIENTS: [Real; 2] = [0.3, -0.45];

    fn modified_craig_sneyd(
        map: SharedMut<dyn FdmLinearOpComposite>,
        bc_set: FdmBoundaryConditionSet,
    ) -> ModifiedCraigSneydScheme {
        let mut scheme = ModifiedCraigSneydScheme::new(THETA, MU, map, bc_set);
        scheme.set_step(DT);
        scheme
    }
    ...
    fn a_step_replays_the_cpp_sequence_on_the_black_scholes_operator() {
        ...
        let mut scheme = modified_craig_sneyd(map, Vec::new());
        ...
        let delta = &y - &u;
        let mut yt = &y0 + &((MU * DT) * &oracle.apply_mixed(&delta));
        yt = &yt + &(((0.5 - MU) * DT) * &oracle.apply(&delta));
        for i in 0..oracle.size() {
            let rhs = &yt - &((THETA * DT) * &oracle.apply_direction(i, &u));
    ...
    /// Closed form: Craig–Sneyd stage 1, then
    /// `y0 + (½ − μ) dt w (y − a)` (mixed term is zero) with a second
    /// directional pass that still reads the input `a`.
    #[test]
    fn a_step_matches_the_closed_form_on_a_diagonal_operator() {
        let mut scheme = modified_craig_sneyd(scaled_composite(&COEFFICIENTS), Vec::new());
        ...
        let mut yt = &y0 + &(((0.5 - MU) * DT * WHOLE) * &(&y - &u));
        for c in COEFFICIENTS {
            yt = &(&yt - &((THETA * DT * c) * &u)) / (1.0 - THETA * DT * c);
        }
    ...
    fn a_step_sets_operator_and_runs_both_bc_apply_cycles() {
        ...
        let mut scheme = modified_craig_sneyd(map, bc_set);
    ...
    fn stepping_before_the_timestep_is_set_fails() {
        let mut scheme =
            ModifiedCraigSneydScheme::new(THETA, MU, scaled_composite(&COEFFICIENTS), Vec::new());
