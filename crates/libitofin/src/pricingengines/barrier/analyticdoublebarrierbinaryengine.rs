...
impl PricingEngine for AnalyticDoubleBarrierBinaryEngine {
    ...
    fn calculate(&mut self) -> QlResult<()> {
        ...
        let cash_payoff = if let Some(ref binary) = arguments.binary_payoff {
        ...
        let vol_ts = self.process.black_volatility().current_link()?;
        let variance = vol_ts.black_variance_date(maturity, 0.0, false)?;
...
mod tests {
    ...
    /// plus 8 degenerate cases.
    #[test]
    fn test_haug_values() {
        let values = [
            // KnockOut
            DoubleBinaryOptionData {
            ...
            // KIKO
            DoubleBinaryOptionData {
                barrier_type: DoubleBarrierType::KIKO,
                barrier_lo: 80.00,
                barrier_hi: 120.00,
                cash: 10.00,
                s: 100.00,
            ...
            // KOKI
            DoubleBinaryOptionData {
                barrier_type: DoubleBarrierType::KOKI,
                barrier_lo: 80.00,
                barrier_hi: 120.00,
                cash: 10.00,
                s: 100.00,
