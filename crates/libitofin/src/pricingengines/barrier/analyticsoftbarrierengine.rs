...
impl AnalyticSoftBarrierEngine {
    ...
    fn validate_inputs(
        ...
        match barrier_type {
            BarrierType::DownIn | BarrierType::DownOut | BarrierType::UpIn | BarrierType::UpOut => {
            }
        }
    ...
    fn standard_barrier_equivalent(&self) -> QlResult<Real> {
        ...
        let mut temp =
            BarrierOption::with_rebate(barrier_type, barrier_hi, 0.0, payoff, exercise, settings)?;
    ...
    fn knock_in_value(
        ...
        let mut term1 = eta * s * ((b - r) * t).exp() * s.powf(-2.0 * mu) * sx.powf(mu + 0.5)
            / (2.0 * (mu + 0.5));
        ...
        let mut term2 = eta * x * (-r * t).exp() * s.powf(-2.0 * (mu - 1.0)) * sx.powf(mu - 0.5)
            / (2.0 * (mu - 0.5));
