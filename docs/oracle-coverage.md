...
| Two-asset spread option (Kirk 1995) | `BasketOption`, `SpreadBasketPayoff`, `KirkEngine` | `basketoption.cpp` `testEuroTwoValues` / `testKirkReferenceValues` | 18-row Haug p.59–60 table for European spread call @ 1e-3 (completing full 57-row testEuroTwoValues); 15-row Mathematica reference table @ 100*EPSILON; rejects non-European exercise, non-spread payoff, and |ρ|>1 |
