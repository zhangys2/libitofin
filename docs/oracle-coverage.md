...
| FD earliest exercise date | `FdBlackScholesVanillaEngine` 200×200 Douglas | `americanoption.cpp` `testFdEarliestExerciseDate` | Deep ITM put: full-window premium > 1 vs European; 3-month window > 0.01 below the full window and > 0.01 above European; price monotone as the window widens |
| FD non-constant rates | `FdBlackScholesVanillaEngine` 200×201 Douglas | `europeanoption.cpp` `testFdEngineWithNonConstantParameters` | Rising forward curve vs analytic European @ 0.01 |
...
| FD Bermudan degeneracy | `FdBlackScholesVanillaEngine` 200×200 Douglas | identity (QL prices no Bermudan through this engine) | Expiry-only Bermudan ≡ European @ 1e-5; monthly exercise > European + 1 and ≤ American; monthly ≥ quarterly |
...
| Binary barrier Haug values | `AnalyticBinaryBarrierEngine` | `binaryoption.cpp` `testCashOrNothingHaugValues` / `testAssetOrNothingHaugValues` | Haug p.180 cash+asset book rows @ 1e-4; cash book-vba q≠0 and touched-barrier extras closed |
| Double-barrier binary Haug values | `DoubleBarrierOption`, `CashOrNothingPayoff`, `AnalyticDoubleBarrierBinaryEngine` | `doublebinaryoption.cpp` `testHaugValues` | Hui (1996) / Haug 2nd Ed. p.181 64 reference values for KnockOut, KnockIn, KIKO, KOKI across 4 barrier pairs and 4 volatility levels @ 1e-4; 8 degenerate boundary cases (spot outside barriers); KnockOut + KnockIn parity vs cash * discount @ 1e-10; exercise type and payoff type validations; binomial and PDE arms deferred |
