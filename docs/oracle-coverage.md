...
| Swaption Black implied vol | `Swaption::implied_volatility` | `swaption.cpp` `testImpliedVolatility` / `testImpliedVolatilityOis` | Spot Physical Black + reduced Spot Cash/`ParYieldCurve` + reduced Forward Physical + reduced OIS Spot/Cash/Forward (incl. Cash×Forward) @ 1e-8; reduced Spot Physical Normal |
