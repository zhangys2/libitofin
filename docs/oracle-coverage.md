...
| Margrabe exchange options (European & American) | `MargrabeOption`, `AnalyticEuropeanMargrabeEngine`, `AnalyticAmericanMargrabeEngine` | `margrabeoption.cpp` `testEuroExchangeTwoAssets` / `testAmericanExchangeTwoAssets` / `testGreeks` | 21-row European table for NPV and 6 greeks @ 1e-3; 18-row American table @ 1e-3; reduced moving-market FD check (delta1/2, gamma1/2, theta, rho); setup_expired zeros greeks |
