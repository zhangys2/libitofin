...
| Double-barrier binary Haug values | `DoubleBarrierOption`, `CashOrNothingPayoff`, `AnalyticDoubleBarrierBinaryEngine` | `doublebarrieroption.cpp` `testHaugValues` | Hui (1996) / Haug 2nd Ed. p.181 32 reference values for KnockOut, KnockIn, KIKO, KOKI across 4 barrier pairs and 4 volatility levels @ 1e-4; 8 degenerate boundary cases (spot outside barriers); KnockOut + KnockIn parity vs cash * discount @ 1e-10; exercise type and payoff type validations |
