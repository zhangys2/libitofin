...
| European Margrabe exchange Greeks | `MargrabeOption`, `AnalyticEuropeanMargrabeEngine` | `margrabeoption.cpp` `testEuroExchangeTwoAssets` / `testGreeks` | 21-row European table for NPV and 6 greeks @ 1e-3; reduced moving-market FD check (delta1/2, gamma1/2, theta, rho); instrument setup_expired zeros greeks; American engine and full testGreeks grid deferred |
