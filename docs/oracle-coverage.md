...
| Equity total return swap | `EquityTotalReturnSwap` + `EquityIndex` + `EquityCashFlow` | `equitytotalreturnswap.cpp` | All 5 test cases (`testFairMargin`, `testErrorWhenNegativeNominal`, `testErrorWhenNoPaymentCalendar`, `testEquityLegNPV`, `testTRSNPV`) across Libor and Overnight floating legs @ 1e-8; fair margin @ 1e-8; zero-gearing supported |
