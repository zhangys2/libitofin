...
| Rates | Vanilla IRS extras | `VanillaSwap` | `swap.cpp` (beyond cached/fair) | true | partial | Cached + fair + notifications + `is_vanilla` + rate/spread dependency + ThirdWednesdayInclusive + `MakeVanillaSwap::with_rule` + in-arrears Hull NPV (`BlackIborCouponPricer` Black76) pinned; stub dates / spot calendar open |
