...
| Rates | Swaption Black extras | `Swaption`, `BlackSwaptionEngine` | `swaption.cpp` | true | partial | Cached NPV/vega/cash-settled + Spot/Forward Physical Black + reduced Spot Cash/`ParYieldCurve` + reduced OIS Spot/Cash/Forward IV + reduced Normal `impliedVolatility` pinned; remaining: full IV grid |
