...
| Rates | Cap/floor Black extras | `CapFloor`, Black engine | `capfloor.cpp` (Bachelier δ, ATM, parity) | true | partial | Cached NPV/vega + Black `impliedVolatility` pinned; Bachelier δ / ATM / parity / `optionlet(i)` open |
...
| Equity | Digitals beyond cash-or-nothing | Digital payoffs + American/MC engines | `digitaloption.cpp`, `binaryoption.cpp` | true | partial | Haug cash-or-nothing + asset-or-nothing; gap/American/MC open |
| Equity | Asset-or-nothing / gap / super payoffs | `AssetOrNothing`, `Gap`, `Super*` | `digitaloption.cpp` | true | partial | Asset-or-nothing European Haug p.90 + FD δ/γ closed; gap/super and `europeanoption.cpp` kk==2 grid open |
