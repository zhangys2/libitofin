...
| Equity | Heston FD scheme grid | `FdHeston*` | `fdheston.cpp` | true | partial | Variance mesher + Black limit + American/Ikonen/div/barrier NPV + ADI convergence (HS/MCS/mod-HS/CS) + Haug barrier-vs-BS table closed. Still open: ExplicitEuler arm, TrBDF2/CN, MethodOfLines/CN, spurious oscillations, American call–put parity, intraday (`QL_HIGH_RESOLUTION_DATE`) |
