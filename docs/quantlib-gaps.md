...
| Rates | Tree / MC / Gaussian1d cap–floor | `TreeCapFloorEngine`, `MCHullWhiteEngine`, … | `capfloor.cpp` | partial | none | `TreeCapFloorEngine` + `DiscretizedCapFloor` (vs Analytic convergence / collar); MC / Gaussian1d / CapHelper `addTimesTo` deferred |
...
| Equity | Forward-start vanilla (non-quanto) | `ForwardVanillaOption`, forward engines | `forwardoption.cpp` | true | partial | Haug NPV + analytic greeks (`testGreeks` / `testPerformanceGreeks`) + greeks-init + BS MC (`testMCPrices`) closed. Still open: Heston MC / analytic-vs-MC |
