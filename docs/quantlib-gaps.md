...
| Rates | Vanilla IRS extras | `VanillaSwap` | `swap.cpp` (beyond cached/fair) | true | partial | Cached + fair + notifications + `is_vanilla` + rate/spread dependency + ThirdWednesdayInclusive + `MakeVanillaSwap::with_rule` + in-arrears Hull NPV (`BlackIborCouponPricer` Black76) + stub first/next-to-last dates pinned; spot calendar open |
...
| Equity | Binary / double-binary barrier | `AnalyticBinaryBarrierEngine`, `AnalyticDoubleBarrierBinaryEngine` | `binaryoption.cpp`, `doublebinaryoption.cpp` | true | partial | `AnalyticBinaryBarrierEngine` (Haug p.180 cash/asset book + cash book-vba q≠0 + touched-barrier extras) and `AnalyticDoubleBarrierBinaryEngine` (Hui 1996 / Haug p.181 64-case KO/KI/KIKO/KOKI + 8 degenerate + in-out parity) closed; `BinomialDoubleBarrierEngine` and PDE arms open |
| Equity | Margrabe / exchange | `MargrabeOption` | `margrabeoption.cpp` | true | partial | European + American engines vs Haug / QL 21-row & 18-row tables @ 1e-3 + reduced moving-market FD check closed. Still open: full testGreeks sweep |
...
| Equity | Basket beyond Choi / single-factor | Kirk/Stulz/Pearson/OperatorSplitting/MC/FD-nD basket | `basketoption.cpp`, `spreadoption.cpp` | true | partial | Choi golden example + Stulz (1982) 39-row min/max European basket table + Kirk (1995) 18-row spread table (completing full 57-row `testEuroTwoValues`) + 15-row `testStrangSplittingSpreadEngineVsMathematica` (Kirk, First-order, and Second-order NPV columns) @ 100*EPSILON + 15-row `testOperatorSplittingSpreadEngine` (First & Second order) + `testNoDivByZeroOperatorSplitting` + Bjerksund & Stensland (2014) closed-form spread engine vs PyFENG reference & parity (`testBjerksundStenslandSpreadEngine`) @ 100*EPSILON + Pearson (1995) 1-D numerical integration spread engine vs parity & Bjerksund-Stensland exchange (`testPearsonSpreadEngine`) closed. Still open: MC/FD-nD basket |
