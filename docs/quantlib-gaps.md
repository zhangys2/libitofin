...
| Equity | Heston FD scheme grid | `FdHeston*` | `fdheston.cpp` | true | partial | Variance mesher + Black limit + American/Ikonen/div/barrier NPV + ADI convergence (HS/MCS/mod-HS/CS) + Haug barrier-vs-BS table + MethodOfLines vs Hundsdorfer closed. Still open: ExplicitEuler arm, 2-D CN/TrBDF2 (#636 ImplicitEuler iterative), spurious oscillations, American call–put parity, intraday (`QL_HIGH_RESOLUTION_DATE`) |
| Equity | Forward-start vanilla (non-quanto) | `ForwardVanillaOption`, forward engines | `forwardoption.cpp` | true | partial | Haug NPV + analytic greeks (`testGreeks` / `testPerformanceGreeks`) closed. Still open: greeks-init, MC, Heston MC / analytic-vs-MC |
