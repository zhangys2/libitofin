...
| Equity | Forward-start vanilla (non-quanto) | `ForwardVanillaOption`, forward engines | `forwardoption.cpp` | true | partial | Haug `testValues` / `testPerformanceValues` closed. Still open: greeks, greeks-init, MC, Heston MC / analytic-vs-MC |
