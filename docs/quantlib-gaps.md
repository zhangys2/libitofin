...
| Rates | Swaption Black extras | `Swaption`, `BlackSwaptionEngine` | `swaption.cpp` (Forward/OIS*) | true | partial | Cached NPV/vega/cash-settled + Spot Physical Black/Normal + Cash/`ParYieldCurve` Black `impliedVolatility` pinned; remaining: Forward / OIS arms |
