...
| Rates | Swaption Black extras | `Swaption`, `BlackSwaptionEngine` | `swaption.cpp` (Bachelier*) | true | partial | Cached NPV/vega/cash-settled + Spot Physical Black `impliedVolatility` pinned; remaining: Bachelier / Cash / Forward / OIS arms |
...
| Equity | Two-asset barrier | `TwoAssetBarrierOption` | `twoassetbarrieroption.cpp` / barrier suite | true | partial | Heynen–Kat analytic Haug 4-row `testHaugValues` @ 4e-3 closed. Knock-in rows / further Haug table deferred |
