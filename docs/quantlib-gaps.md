...
| Rates | Cap/floor Black extras | `CapFloor`, Black engine | `optionletsVega` / `optionletsStdDev`, `deepUpdate`, bindings | true | partial | Cached NPV/vega + δ + `impliedVolatility` + ATM/parity + `optionlet(i)` recomposition pinned; remaining: vega/stddev vectors, `deepUpdate`, Python/FFI `optionlet` |
...
| Equity | Digitals beyond cash-or-nothing | Digital payoffs + American/MC engines | `digitaloption.cpp`, `binaryoption.cpp` | true | partial | Haug cash-or-nothing + asset-or-nothing + gap; `AnalyticDigitalAmericanEngine` at-hit and at-expiry knock-in cash/asset @ 1e-4; KO / MC deferred |
