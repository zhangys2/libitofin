...
| Rates | Swaption Black extras | `Swaption`, `BlackSwaptionEngine` | `swaption.cpp` | true | partial | Cached NPV/vega/cash-settled + Spot/Forward Physical Black + reduced Spot Cash/`ParYieldCurve` + reduced OIS Spot/Cash/Forward (incl. Cash×Forward) IV + reduced Normal `impliedVolatility` pinned; remaining: full IV grid |
...
| Equity | Extensible options | Holder/writer extensible | `extensibleoptions.cpp` | true | partial | Writer analytic Haug call @ 1e-4 (`testAnalyticWriterExtensibleOptionEngine`) closed, plus independent put and q≠0 call pins. Holder engine deferred. |
