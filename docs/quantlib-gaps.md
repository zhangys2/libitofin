...
| Equity | Heston FD scheme grid | `FdHeston*` | `fdheston.cpp` | true | partial | Variance mesher + Black limit + American/Ikonen/div/barrier NPV + ADI convergence (HS/MCS/mod-HS/CS) closed. Still open: ExplicitEuler arm, TrBDF2/CN, Haug barrier-vs-BS table, MethodOfLines/CN, spurious oscillations, American call–put parity, intraday (`QL_HIGH_RESOLUTION_DATE`) |
