...
| Rates | Cap/floor Black extras | `CapFloor`, Black engine | `capfloor.cpp` (Bachelier δ, ATM, parity) | true | partial | Cached NPV/vega + `optionletsDelta` + Black/`Normal` `impliedVolatility` pinned; Bachelier δ / ATM / parity / `optionlet(i)` open |
