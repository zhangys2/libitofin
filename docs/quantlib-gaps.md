...
| Equity | Binary / double-binary barrier | `AnalyticBinaryBarrierEngine`, `AnalyticDoubleBarrierBinaryEngine` | `binaryoption.cpp`, `doublebarrieroption.cpp` | true | full | `AnalyticBinaryBarrierEngine` (Haug p.180 cash/asset book + cash book-vba q≠0 + touched-barrier extras) and `AnalyticDoubleBarrierBinaryEngine` (Hui 1996 / Haug p.181 32-case KO/KI/KIKO/KOKI + 8 degenerate + in-out parity) closed |
