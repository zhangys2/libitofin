...
| Rates | Cap/floor Black extras | `CapFloor`, Black engine | `capfloor.cpp` (ATM, parity, `optionlet(i)`) | true | partial | Cached NPV/vega + Black/`Normal` δ + `impliedVolatility` pinned; ATM/parity suite already covered; `optionlet(i)` open |
