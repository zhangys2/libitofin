...
| Equity | Basket beyond Choi / single-factor | Kirk/Stulz/Pearson/MC/FD-nD basket | `basketoption.cpp`, `spreadoption.cpp` | true | partial | Choi golden example + Stulz (1982) 39-row min/max European basket table + Kirk (1995) 18-row spread table (completing full 57-row `testEuroTwoValues`) + 15-row `testStrangSplittingSpreadEngineVsMathematica` (Kirk NPV column) @ 100*EPSILON + Bjerksund & Stensland (2014) closed-form spread engine vs PyFENG reference & parity (`testBjerksundStenslandSpreadEngine`) @ 100*EPSILON closed. Still open: Pearson/MC/FD-nD basket, Strang/OS columns |
