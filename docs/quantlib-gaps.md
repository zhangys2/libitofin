...
| Rates | Swaption Black extras | `Swaption`, `BlackSwaptionEngine` | `swaption.cpp` (Forward/OIS*) | true | partial | Cached NPV/vega/cash-settled + Spot Physical Black + reduced Spot Cash/`ParYieldCurve` + reduced Normal `impliedVolatility` pinned; remaining: Forward / OIS arms |
