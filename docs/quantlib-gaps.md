...
| Rates | Cap/floor Black extras | `CapFloor`, Black engine | `deepUpdate`, bindings | true | partial | Cached NPV/vega + δ + `impliedVolatility` + ATM/parity + `optionlet(i)` + `optionletsVega`/`StdDev` pinned; remaining: `deepUpdate`, Python/FFI `optionlet` |
| Rates | Swaption Black extras | `Swaption`, `BlackSwaptionEngine` | `swaption.cpp` | true | partial | Cached NPV/vega/cash-settled + Spot/Forward Physical Black + reduced Spot Cash/`ParYieldCurve` + reduced Spot Physical OIS + reduced Normal `impliedVolatility` pinned; remaining: OIS Cash/Forward arms, full IV grid |
...
| Equity | Extensible options | Holder/writer extensible | `extensibleoptions.cpp` | true | partial | Writer analytic Haug call @ 1e-4 closed (put + q≠0 pins). Holder analytic Haug call @ 1e-4 (`testAnalyticHolderExtensibleOptionEngine`) closed, plus independent q≠0 call pin. QL suite has no holder put. |
